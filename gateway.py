...
class QQBotGateway:
    ...
    async def start(self) -> None:
        ...
        if not cfg.get("enabled") or cfg.get("transport") not in {"websocket", "both"} or self._task:
            return
        self._stop.clear()
    ...
    async def _run(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                ...
                logger.warning("QQ Bot 网关断开: %s", exc)
        ...
        async with websockets.connect(url, ping_interval=None) as ws:
            ...
            try:
                async for raw in ws:
                    ...
                    if payload.get("op") == 0:
                        await handle_event(str(payload.get("t") or ""), payload.get("d") or {}, payload.get("id"))
                    if payload.get("op") == 7:
                        return
            finally:
                heartbeat.cancel()
